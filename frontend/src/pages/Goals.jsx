        ...
        <header className="flex-shrink-0 px-8 py-8 md:px-12 bg-background-dark z-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-1 md:gap-3">
              <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl font-extrabold text-text-main tracking-tight">
                Goal Setting
              </h2>
              <div className="flex items-center gap-2 text-text-secondary">
                <span className="text-xs sm:text-sm font-semibold tracking-wide uppercase text-text-secondary">
                  Weekly Focus
                </span>
          ...
          <button
            ...
            className="bg-primary hover:bg-primary-hover text-background-dark font-bold py-2.5 px-4 sm:py-3 sm:px-6 md:py-3.5 md:px-7 rounded-xl flex items-center justify-center gap-1 sm:gap-2 transition-all shadow-glow hover:-translate-y-1 text-sm sm:text-base"
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
              <div className="lg:col-span-7 p-6 sm:p-8 lg:p-8 rounded-2xl sm:rounded-3xl bg-surface border border-border-subtle shadow-card relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:opacity-10 transition-opacity duration-700 pointer-events-none">
                  <span className="material-symbols-outlined text-[150px] sm:text-[200px] text-text-main">
                    emoji_events
                  </span>
                </div>
                <div className="flex flex-col h-full justify-between relative z-10">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6 sm:gap-8 mb-8 sm:mb-12">
                    <div>
                      <h3 className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-text-secondary mb-2 sm:mb-3">
                        Overall Progress
                      </h3>
                      <div className="flex items-baseline gap-2 sm:gap-4">
                        <span className="text-5xl sm:text-6xl md:text-6xl lg:text-7xl font-black text-text-main tracking-tighter leading-none">
                          {overallProgress}%
                        </span>
                      </div>
                    </div>
                    <div className="flex flex-col sm:items-end gap-1">
                      ...
                      <span className="text-[10px] sm:text-xs text-text-secondary font-medium mt-1">
                        vs last week
                      </span>
                    </div>
                  </div>
              ...
              <div className="lg:col-span-5 flex flex-col gap-4">
                <div className="flex-1 p-6 rounded-2xl sm:rounded-3xl bg-primary text-background-dark shadow-glow flex items-center justify-between group cursor-default">
                  <div>
                    <p className="text-[10px] sm:text-xs font-bold uppercase tracking-widest opacity-60 mb-2">
                      Current Streak
                    </p>
                ...
                <div className="flex-1 p-6 rounded-2xl sm:rounded-3xl bg-surface border border-border-subtle shadow-card flex items-center justify-between group hover:border-text-secondary transition-colors">
                  <div>
                    <p className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-text-secondary mb-2">
                      Goals Met
                    </p>
                    <div className="flex items-baseline gap-2">
                      <p className="text-4xl sm:text-4xl font-black text-text-main">
                        {completedGoals}
                      </p>
            ...
            <div className="flex flex-col gap-6 sm:gap-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {filteredGoals.map((goal) => (
                  <div
                    key={goal.id}
                    className={`group relative flex flex-col p-5 rounded-2xl sm:rounded-3xl ${
                      goal.completed
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="size-7 sm:size-8 flex items-center justify-center rounded-full hover:bg-surface-highlight text-text-secondary transition-colors"
                          >
                    ...
                    <div className="mt-4 sm:mt-6">
                      <h3
                        className={`text-xl sm:text-2xl font-bold tracking-tight group-hover:text-primary ${
                          goal.completed
                      ...
                      <div className="flex justify-between items-end">
                        <span
                          className={`text-3xl sm:text-4xl font-bold tracking-tighter ${
                            goal.completed ? "text-primary" : "text-text-main"
                          }`}
                        >
                          {goal.current}
                          <span className="text-sm sm:text-base font-semibold text-text-secondary ml-1">
                            {goal.unit}
                          </span>
                        </span>
                      ...
                      <div className="w-full h-1.5 sm:h-2 bg-surface-highlight rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${
                            goal.completed ? "bg-primary" : "bg-text-main"
                          }`}
                ...
                <div
                  onClick={() => setShowModal(true)}
                  className="group flex flex-col justify-center items-center p-5 sm:p-6 lg:p-7 rounded-2xl sm:rounded-3xl border-2 border-dashed border-border-subtle hover:border-text-secondary hover:bg-surface-highlight/50 cursor-pointer transition-all"
                >
                  ...
                  <p className="text-base sm:text-lg font-bold text-text-secondary group-hover:text-text-main transition-colors">
                    Create New Goal
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-4 sm:gap-6">
              ...
              <div className="hidden md:block overflow-x-auto rounded-2xl sm:rounded-3xl border border-border-subtle bg-surface shadow-card">
                <table className="w-full text-left text-xs sm:text-sm">
                  <thead className="bg-surface-highlight/50 text-text-secondary uppercase tracking-wider text-[10px] sm:text-[11px] font-bold">
                    <tr>
                      <th className="px-4 sm:px-6 md:px-8 py-4 sm:py-5 rounded-tl-2xl sm:rounded-tl-3xl">
                        Activity Name
                      </th>
                      ...
                      <th className="px-4 sm:px-6 md:px-8 py-4 sm:py-5 text-right rounded-tr-2xl sm:rounded-tr-3xl">
                        Status
                      </th>
                    </tr>
                  </thead>
                    ...
                    <tr className="hover:bg-surface-highlight/30 transition-colors group">
                      ...
                      <td className="px-4 sm:px-6 md:px-8 py-4 sm:py-6">
                        <div className="flex flex-col">
                          <span className="font-medium text-text-main text-xs sm:text-sm">
                            Today
                          </span>
                      ...
                      <td className="px-4 sm:px-6 md:px-8 py-4 sm:py-6">
                        <span className="text-text-main font-bold bg-surface-highlight px-2 py-1 rounded text-[10px] sm:text-xs">
                          3.2 km
                        </span>
                      </td>
                      <td className="px-4 sm:px-6 md:px-8 py-4 sm:py-6 text-right">
                        <span className="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold bg-primary/10 text-primary border border-primary/20">
                          <span className="size-1 sm:size-1.5 rounded-full bg-primary animate-pulse"></span>{" "}
                          Completed
                        </span>
                      </td>
                    </tr>
              ...
              <div className="md:hidden space-y-3">
                <div className="bg-surface border border-border-subtle rounded-2xl p-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-text-main font-bold bg-surface-highlight px-2 py-1 rounded text-xs">
                      3.2 km
                    </span>
                  </div>
                </div>
